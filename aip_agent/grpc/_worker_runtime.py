...
class HostConnection:
    ...
    def metadata(self) -> Sequence[Tuple[str, str]]:
        ...
        if timestamp - int(self._time) > 180: 
            ...
            signature = create_auth(timestamp)
            self._sign = signature
        return [("client-id", self._client_id), ("sign", self._sign), ("timestamp", self._time), ("agent", self._membase_id)]
...
class GrpcWorkerAgentRuntime(AgentRuntime):
    ...
    async def _run_read_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                logger.info("message type: %s", oneofcase)
                match oneofcase:
                    case "request":
    ...
    async def _process_event(self, event: cloudevent_pb2.CloudEvent) -> None:
        ...
        logging.debug(f"=== handle cloud message: {recipients} {message}")
