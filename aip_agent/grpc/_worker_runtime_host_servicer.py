...
class GrpcWorkerAgentRuntimeHostServicer(agent_worker_pb2_grpc.AgentRpcServicer):
    ...
    async def OpenChannel(  # type: ignore
        ...
        logging.debug(f"openchannel: {client_id}")
    ...
    async def OpenControlChannel(  # type: ignore
        ...
        client_id = await get_client_id_or_abort(context)
        logging.debug(f"opencontrolchannel: {client_id}")
    ...
    async def RegisterAgent(  # type: ignore
        ...
        logging.debug(f"register agent: {client_id} {request.type}")
        async with self._agent_type_to_client_id_lock:
    ...
    async def AddSubscription(  # type: ignore
        ...
        logging.debug(f"addsub: {client_id}")
        

        subscription = subscription_from_proto(request.subscription)
        logging.debug(f"addsub: {client_id} {subscription.id}")
        if isinstance(subscription, TypePrefixSubscription):
            logging.debug(f"addsub: {client_id}, topic: {subscription.topic_type_prefix}, agent: {subscription.agent_type}")
        if isinstance(subscription, TypeSubscription):
            logging.debug(f"addsub: {client_id}, topic: {subscription.topic_type}, agent: {subscription.agent_type}") 
    ...
    async def RemoveSubscription(  # type: ignore
        ...
        logging.debug(f"removesub: {_client_id}")
